...
exports.trainStatus = onRequest(async (req, res) => {
  ...
  try {
    ...
    if (!API_KEY) {
      return res.status(500).json({
        success: false,
        error: "TRAIN_API_KEY not set in environment"
      });
    }

    // Example provider (replace with real one like RapidAPI Indian Rail API)
    const url = `https://api.railwayapi.com/v2/live/train/${trainNo}/apikey/${API_KEY}/`;
